...
import com.example.chromecastone.Adapter.ListPhotosAdapter;
...
import com.example.chromecastone.databinding.ActivityListPhotosBinding;
...
public class ListPhotosActivity extends AppCompatActivity implements ItemOnClickListener, DeviceConnectListener {

    private ListPhotosAdapter adapter;
    ...
    private final ActivityResultLauncher<Intent> intentActivityResultLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), new ActivityResultCallback<ActivityResult>() {
        ...
        public void onActivityResult(ActivityResult activityResult) {
            ...
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    ListPhotosActivity.this.startActivityPlay(intExtra);
                }
            }, 500L);
        }
    });
    ...
    public void onCreate(Bundle bundle) {
        ...
        ActivityListPhotosBinding inflate = ActivityListPhotosBinding.inflate(getLayoutInflater());
        ...
        findViewById(R.id.cast_photos).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new CastDeviceListActivity().setDeviceConnectListener(ListPhotosActivity.this);
                startActivityForResult(new Intent(ListPhotosActivity.this, CastDeviceListActivity.class), 100);
            }
        });
    }
    ...
    private void initMain() {
        try {
            if (this.listImages.size() != 0) {
                ...
                ListPhotosAdapter listPhotosAdapter = new ListPhotosAdapter(this);
                this.adapter = listPhotosAdapter;
                listPhotosAdapter.setItemOnClickListener(this);
                this.binding.pbLoad.setVisibility(View.GONE);
                this.binding.rvListdataPhotos.setLayoutManager(new GridLayoutManager(this, 3));
                this.adapter.setListImageData(this.listImages);
                this.binding.rvListdataPhotos.setAdapter(this.adapter);
            } else {
                onBackPressed();
            }
        } catch (Exception e) {
    ...
    private class MySessionManagerListener implements SessionManagerListener<CastSession> {
        ...
        public void onSessionEnded(CastSession castSession, int i) {
            if (castSession == ListPhotosActivity.this.mCastSession) {
                ListPhotosActivity.this.mCastSession = null;
            }
            ListPhotosActivity.this.invalidateOptionsMenu();
        }

        @Override
        public void onSessionResumed(CastSession castSession, boolean z) {
            ListPhotosActivity.this.mCastSession = castSession;
            ListPhotosActivity.this.invalidateOptionsMenu();
        }
